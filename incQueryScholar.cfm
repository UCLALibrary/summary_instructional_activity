<cfquery name="Sessions" datasource="#APPLICATION.dsn#">
	SELECT DISTINCT
		...
		END AS Department,
		--Coalesce(lcl.LearnerCategory, 'N/A') AS LearnerCategory,
		--Coalesce(dbo.build_learner_cats(s.SessionID), 'N/A') AS LearnerCategory,
		Case
			When len(dbo.build_learner_cats(s.SessionID)) <> 0 Then dbo.build_learner_cats(s.SessionID)
			Else '&nbsp;'
		End AS LearnerCategory,
	...
	WHERE
		...
		AND s.ScholarlyCommunication = 1
